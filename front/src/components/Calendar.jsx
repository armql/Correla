...
export default function Calendar() {
  ...
  const [isSelectedDays, setIsSelectedDays] = useState({}); // State to track the selected day for each month
  ...
  const isPastDay = (day, currentMonth) => {
    ...
    return (
      ...
      (year === currentYear &&
        month === currentMonth &&
        (day < currentDate.getDate() || isSelectedDays[currentMonth] > day))
    );
  };

  const isCurrentDay = (day, currentMonth) => {
    return (
  ...
  const changeDay = (day, currentMonth) => {
    if (day <= getDaysInMonth(year, currentMonth).length) {
      const newIsSelectedDays = { ...isSelectedDays, [currentMonth]: day };
      setIsSelectedDays(newIsSelectedDays);
    }
  };
            ...
            <div className="flex flex-row gap-1 justify-center items-center">
              <div className="flex gap-2 bg-white border border-gray-300 font-light text-sm py-1.5 px-4 hover:bg-gray-50 cursor-pointer rounded-md shadow-sm">
                <div className="flex justify-center items-center">
                  <button
                    onClick={() => changeDay(selectedDate.getDate() - 1, month)}
                  >
                    <svg
                  ...
                  <li
                    ...
                    className={`py-3 tracking-wide font-light px-6 hover:bg-gray-50 ${
                      isPastDay(day, index)
                        ...
                        ? "bg-yellow-200"
                        : day === isSelectedDays[index]
                        ? "bg-gray-500"
                        : ""
                    } border border-transparent hover:border-gray-200 hover:cursor-pointer rounded-sm shadow-sm`}
